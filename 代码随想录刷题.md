...
##### 题目1：leetcode1002查找共用字符

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220514214347224.png" alt="image-20220514214347224" style="zoom:80%;" />

求两个字符串重合的部分：用一个数组记录第一个字符串所有出现的字母，之后第二个数组记录第二个字符串所有出现的字母及个数，如果第一个数组的某一位高于第二个数组记录的数组就用第二个替换，而第二个数组比第一个数字高就不用管。这样就可以记录到所有重复的部分了
...
class Solution {
    public List<String> commonChars(String[] words) {
        ...
        for(int j=0;j<26;j++){
            while(recordArr[j]>0){
                res.add(Character.toString(j+'a'));
                recordArr[j]--;
            }
        }
        return res;
...
我们使用数组当哈希表记录的原因是我们数据的范围被规定在了特定区间，例如都是小写字母这样的区间，二如果我们存储的数太大，并且数据没有范围的时候这时候我们需要使用set判断是否包含某个元素。

使用set的缺点：当我们使用了set意味着每一个值都需要经过哈希函数的计算讲数值映射到set的索引，这需要耗费算力。所以当数据值大小固定的时候我们更喜欢用数组去记录。

##### 题目1：leetcode349：两个数组的交集

数组中存的数据可能从1到无穷大，如果使用数据当索引做一个记录数组这时候数组大小是无穷大，这是不行的，所以使用hashset。

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220514224625972.png" alt="image-20220514224625972" style="zoom:80%;" />
...
当题目中需要判断是否有重复元素的时候需要想到哈希表。

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220515172310449.png" alt="image-20220515172310449" style="zoom:80%;" />

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220515172326186.png" alt="image-20220515172326186" style="zoom:80%;" />
...
<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220516161223910.png" alt="image-20220516161223910" style="zoom:80%;" />

解：把target-nums中的各个元素当作key，把索引下标当作value
...
双指针：双指针法如同查找算法一样，需要在有序的数据集中进行搜寻，这是因为有序的数据集可以方便我们处理指针移动的逻辑。如果指针每移动一步有n种可能，那么只能使用for循环去暴力尝试每一种，但是如果指针的移动是有秩序的，那么这种情况将会好很多。

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220517160738687.png" alt="image-20220517160738687" style="zoom:80%;" />
...
##### 题目3：leetcode16最接近的三数之和

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220517161653526.png" alt="image-20220517161653526" style="zoom:80%;" />

这道题也是需要先排序之后使用双指针法，不能使用滑动窗口的原因是，我们需要的元素集合并不总是再一个窗口中。滑动窗口题目一般都有**连续数组**这样的描述，这就表示了所需要的元素必定是再一个集合中的。但是这道题并不是如此，我们需要的元素组合的下标可能是0，1，3。123，024.这样的组合。

双指针的移动只和三个数之和与target之间的大小有关，和gap无关
...
反转字符串中我们经常需要把两个字符换位置，但是字符串本身是不能变的，所以我们有两种方式对其位置进行交换

- 字符串变成字符数组String变成Char[]
- 字符串变成Stringbuffer即不可变字符串变成可变的字符串，之后通过stringbuffer.setCharAt（index，c）对其进行变换

##### 题目1：leetcode344反转字符串

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220518150059151.png" alt="image-20220518150059151" style="zoom:80%;" />

编写交换函数，用双指针从头和尾到中间一直遍历即可
...
##### 题目2：leetcode541反转字符串2

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220518150829726.png" alt="image-20220518150829726" style="zoom:80%;" />
...
class Solution {
    public String reverseStr(String s, int k) {
        ...
        int len = string.length;
        while(len>=range2k){
            swapInrange(string,start,rangeK-1);
            start = range2k;
            rangeK =start+k;
            range2k =rangeK+k;
        }
        ...
        if(len<=rangeK){
            swapInrange(string,start,len-1);
        }
        return new String(string);
    }
    public void swap(char[] s,int index1,int index2){
    ...
    public void swapInrange(char[] string,int left,int right){
        while(left<right){
            swap(string,left,right);
            right--;
            left++;
...
（如果字符数组想转换成字符串，使用string的构造函数，把char[]丢进去即可）

##### 题目3：反转字符串中的元音字母

<img src="C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220518162344944.png" alt="image-20220518162344944" style="zoom:80%;" />
...
##### 题目4：leetcode151删除字符串中的单词

- 字符串相关API有StringBuffer，StringBuilder（这两个直接toString即可）其作用相当于动态字符串。stringBuffer也有charAt（int）方法，它可以取得指定索引位置上的字符，我们可以通过setChatAt（int index，char c）来将字符c插入到指定的位置上去。

- s.charAt(int index)这个方法是可以相当于数组的用法的，我们将需要的索引放入方法需要传递的参数中，返回的即是我们需要的字符。配合指针的使用即可对应数组的索引。

- toCharArray，这个方法是对应将字符串转换成一个字符数组的方式。在字符数组中进行操作后我们可以使用new String（char[] charArr）进行新的字符串的创建。

- split函数，这是一个实例化对象可以使用的方法，其构造函数传递按哪个字符分割的，返回值为一个字符串数组
...
![image-20220519161726857](C:\Users\admin\AppData\Roaming\Typora\typora-user-images\image-20220519161726857.png)
...
class Solution {
    public String reverseWords(String s) {
        StringBuffer sb = solveSpace(s);
        int startIndex = 0;
        int endIndex = sb.length()-1;
        StringBuffer sbreverseFirst = reverseInRange(sb,startIndex,endIndex);
        int startPosition = 0;
        int endPosition = 0;
        while(endPosition<sbreverseFirst.length()){
            if(sbreverseFirst.charAt(endPosition)!=' '){
                endPosition++;
            }else{
                reverseInRange(sbreverseFirst,startPosition,endPosition-1);
                startPosition=endPosition+1;
                endPosition++;
            }
            if(endPosition==sbreverseFirst.length()-1){
                reverseInRange(sbreverseFirst,startPosition,endPosition);
            }
        }
        return sbreverseFirst.toString();
    }
    public StringBuffer reverseInRange(StringBuffer s,int startIndex,int endIndex){
    ...
    public StringBuffer solveSpace(String s){
        StringBuffer res = new StringBuffer();
        int startIndex = 0;
        int endIndex = s.length()-1;
        while(s.charAt(startIndex)==' '){
